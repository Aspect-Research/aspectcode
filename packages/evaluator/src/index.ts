...
export async function evaluate(options: {
  ...
  // Step 2: Run probes
  const probeResults = await runProbes(agentsContent, probes, provider, fileContents, log, signal);
  const failures = probeResults.filter((r) => !r.passed);

  // Step 3: Diagnose if there are failures
  let diagnosis: EvaluationResult['diagnosis'];
  if (failures.length > 0) {
    diagnosis = await diagnose(failures, agentsContent, provider, log, signal);
  }
