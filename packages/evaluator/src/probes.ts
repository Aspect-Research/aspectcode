...
import type { Probe, ProbeGeneratorOptions, HarvestedPrompt } from './types';

// ── KB section parsers ──────────────────────────────────────

/** Extract a section from KB text by heading prefix. */
export function extractSection(kb: string, heading: string): string {
...
export function parseHubs(architecture: string): Array<{ file: string; inDegree: number; outDegree: number }> {
...
export function parseEntryPoints(architecture: string): Array<{ file: string; kind: string }> {
...
export function parseConventions(mapSection: string): string[] {
