...
function readVscdbKey(dbPath: string, key: string): string | undefined {
  ...
  let db: { prepare: (sql: string) => { get: (key: string) => { value?: string } | undefined }; close: () => void } | undefined;
  try {
...
function readVscdbKeysLike(dbPath: string, pattern: string): Array<{ key: string; value: string }> {
  ...
  let db: { prepare: (sql: string) => { all: (pattern: string) => Array<{ key: string; value: string }> }; close: () => void } | undefined;
  try {
    db = new Database(dbPath, { readonly: true, fileMustExist: true });
...
function parseCopilotSessionData(data: unknown, root: string): HarvestedPrompt[] {
  const prompts: HarvestedPrompt[] = [];

  // Handle array of sessions or single session
  const sessions: Array<Record<string, unknown>> = Array.isArray(data) ? data : [data as Record<string, unknown>];
  for (const session of sessions) {
    const requests = (session.requests || session.entries || []) as Array<Record<string, unknown>>;
    for (const req of requests) {
      const userText = (req.message || req.text || req.prompt || '') as string;
      const resp = req.response as Record<string, unknown> | undefined;
      const result = req.result as Record<string, unknown> | undefined;
      const assistantText = (resp?.value || resp?.text || result?.text || '') as string;

      if (!userText || !assistantText) continue;

      const fileRefs: string[] = [];
      if (Array.isArray(req.references)) {
        for (const ref of req.references as Array<Record<string, unknown>>) {
      ...
      prompts.push({
        source: 'copilot-chat',
        timestamp: req.timestamp ? new Date(req.timestamp as number).toISOString() : undefined,
        userPrompt: userText,
...
function parseCopilotConversationsJson(data: Record<string, unknown>, root: string): HarvestedPrompt[] {
...
function parseCursorComposer(data: Record<string, unknown>, root: string): HarvestedPrompt[] {
  const prompts: HarvestedPrompt[] = [];
  const composers = (data.allComposers || data.composers || []) as Array<Record<string, unknown>>;

  for (const composer of composers) {
    const conversation = (composer.conversation || []) as Array<Record<string, unknown>>;
    for (let i = 0; i < conversation.length; i++) {
      ...
      const assistantText = (next.text || '') as string;
      if (!userText || !assistantText) continue;
...
function parseCursorPanelChats(data: Record<string, unknown>, root: string): HarvestedPrompt[] {
  const prompts: HarvestedPrompt[] = [];
  const panels = (data.panelChats || data.chats || []) as Array<Record<string, unknown>>;

  for (const panel of panels) {
    if (panel.isDeleted) continue;
    const messages = (panel.messages || []) as Array<Record<string, unknown>>;
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      const assistantText = (next.content || '') as string;
      if (!userText || !assistantText) continue;
...
function extractCursorContextFiles(context: unknown): string[] {
  if (!context) return [];
  ...
  if (Array.isArray(ctx.selections)) {
    for (const s of ctx.selections as Array<Record<string, unknown>>) {
...
function parseWindsurfConversations(data: unknown, root: string): HarvestedPrompt[] {
  const prompts: HarvestedPrompt[] = [];
  const obj = data as Record<string, unknown>;

  // Try multiple possible structures
  const conversations = (obj.conversations || obj.chats || (Array.isArray(data) ? data : [data])) as Array<Record<string, unknown>>;

  for (const conv of conversations) {
    const messages = (conv.messages || conv.turns || []) as Array<Record<string, unknown>>;
    for (let i = 0; i < messages.length; i++) {
      const msg = messages[i];
      if (msg.role !== 'user') continue;
      const next = messages[i + 1];
      ...
      const userText = (msg.content || msg.text || '') as string;
      const assistantText = (next.content || next.text || '') as string;
      if (!userText || !assistantText) continue;
