...
import { extractFilePaths, filterRecent, vscodeGlobalStoragePath, noopLogger, DEFAULT_MAX_PER_SOURCE } from './common';
 ...
 */
function parseClineHistory(filePath: string, taskId: string, root: string): HarvestedPrompt[] {
  const content = fs.readFileSync(filePath, 'utf-8');
  const messages = JSON.parse(content) as Array<Record<string, unknown>>;
 ...
 */
function extractTextFromClineContent(content: unknown): string {
...
function isToolResultOnly(content: unknown): boolean {
  ...
  return content.every((b: Record<string, unknown>) => b.type === 'tool_result');
}
