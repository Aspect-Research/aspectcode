...
async function runOnce(
  ...
  let discoveryProvider: import('@aspectcode/optimizer').LlmProvider | undefined;
  try {
    const env = loadEnvFile(root);
    const creds = loadCredentials();
    if (creds) env['ASPECTCODE_CLI_TOKEN'] = creds.token;
    if (config?.apiKey && !env['ASPECTCODE_LLM_KEY']) env['ASPECTCODE_LLM_KEY'] = config.apiKey;
  ...
  const fileCount = workspace.relativeFiles.size;
  let hintTimer: ReturnType<typeof setTimeout> | undefined;
  if (fileCount > 3000) {
    hintTimer = setTimeout(() => {
      store.setPhase('analyzing', `${fileCount.toLocaleString()} files — this could take a while`);
    }, 8000);
  }
  ...
  // Preserve our AGENTS.md on subsequent runs (probe-and-refine edits, dream refinements).
  // If the file exists but wasn't generated by us, use our rendered base instead.
  const agentsExists = fs.existsSync(agentsPath);
  const renderedBase = renderAgentsMd(model, path.basename(root));
  const existingIsOurs = agentsExists && (() => {
    try {
      const content = fs.readFileSync(agentsPath, 'utf-8');
      return content.includes('aspectcode.com') || content.includes('aspectcode:');
    } catch { return false; }
  })();
  const baseContent = existingIsOurs
...
export async function runPipeline(ctx: RunContext): Promise<ExitCodeValue> {
  ...
  // ── Initial run — probe-and-refine on first run or if AGENTS.md wasn't generated by us
  const agentsFile = path.join(root, 'AGENTS.md');
  const existingAgents = fs.existsSync(agentsFile) ? fs.readFileSync(agentsFile, 'utf-8') : '';
  const isOurFile = existingAgents.includes('aspectcode.com') || existingAgents.includes('aspectcode:');
  const initialProbeAndRefine = !isOurFile;
